...
#         # 生成一个 d_model 维随机向量
#         random_vector = np.random.rand(d_model)
        
#         # 以 random_vector 作为第一行和第一列生成 Toeplitz 矩阵
#         toeplitz_matrix = toeplitz(random_vector, random_vector)
        
#         # 取 Toeplitz 矩阵的前 d_vocab 行
#         toeplitz_matrix = toeplitz_matrix[:,:d_vocab] / np.sqrt(d_model)
        
#         # 将 Toeplitz 矩阵转换为 torch 的 nn.Parameter
#         self.W_E = nn.Parameter(t.tensor(toeplitz_matrix, dtype=t.float32))
    # def forward(self, x):
...
def calculate_embedding_diff(embed_matrix, a, b, c, d):
    ...
    if b > embed_matrix.shape[1] or d > embed_matrix.shape[1]:
        raise ValueError("索引范围超过嵌入矩阵的维度。")

    diffs1 = t.norm(embed_matrix[:, a+1:b] - embed_matrix[:, a:b-1], dim=0)
    diff1_mean = (b-a)*diffs1.mean().item()

    diffs2 = t.norm(embed_matrix[:, c+1:d] - embed_matrix[:, c:d-1], dim=0)
    diff2_mean = (d-c)*diffs2.mean().item()

    return diff1_mean+diff2_mean
...
class Trainer:
    ...
    def plot_metrics(self):
        ...
        if not os.path.exists('plots'):
            os.makedirs('plots')
        strname = 'metrics_' + str(config.p) + '_' + str(config.d) + '_' + str(config.d_model) + '_' + str(config.frac_train)
        if config.bol_pos: 
            figname = 'plotfigure/' + strname + '_pos.png'
        else:
            figname = 'plotfigure/' + strname + '.png'
        ...
        if config.bol_pos:
            csvname = 'exresult/' + strname + '_pos_data.csv'
        else:
            csvname = 'exresult/' + strname + '_data.csv'
        data_to_save.to_csv(csvname, index=False)  # 保存到CSV文件
