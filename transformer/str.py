...
def gen_train_test(config):
    ...
    test_pairs = [(i, j + config.d, config.d_vocab - 1) for i in range(k + 1) for j in range( config.p)]
...
def apply_gaussian_filter(data, sigma=2):
    ...
    使用高斯滤波对数据进行平滑
    :param data: 输入数据
    :param sigma: 高斯函数的标准差，值越大平滑效果越明显
    :return: 平滑后的数据
    """
...
class Trainer:
    ...
    def plot_metrics(self):
        ...
        if not os.path.exists('plots'):
            os.makedirs('plots')
        strname = 'testplots/ranmetrics_' + str(config.p) + '_' + str(config.d) + '_' + str(config.d_model) + '_' + str(config.frac_train)
        if config.bol_pos: 
            figname = strname + '_pos.png'
        else:
            figname = strname + '.png'
        ...
        if config.bol_pos:
            csvname = strname + '_pos_data.csv'
        else:
            csvname = strname + '_data.csv'
